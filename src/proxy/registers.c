...
// =====================================================================
//  ImuShadow  — IMU shadow register (was Lsm6dsv16xReg)
// =====================================================================
...
ImuShadow g_imu_shadow = {0};

/* ── Read access ── */

int16_t  imu_get_qw(void)              { return g_imu_shadow.qw; }
int16_t  imu_get_qx(void)              { return g_imu_shadow.qx; }
int16_t  imu_get_qy(void)              { return g_imu_shadow.qy; }
int16_t  imu_get_qz(void)              { return g_imu_shadow.qz; }

int16_t  imu_get_gx(void)              { return g_imu_shadow.gx; }
int16_t  imu_get_gy(void)              { return g_imu_shadow.gy; }
int16_t  imu_get_gz(void)              { return g_imu_shadow.gz; }

int16_t  imu_get_ax(void)              { return g_imu_shadow.ax; }
int16_t  imu_get_ay(void)              { return g_imu_shadow.ay; }
int16_t  imu_get_az(void)              { return g_imu_shadow.az; }

int16_t  imu_get_temp(void)            { return g_imu_shadow.temp; }

uint32_t imu_get_timestamp_ms(void)    { return g_imu_shadow.timestamp_ms; }
uint32_t imu_get_sample_count(void)    { return g_imu_shadow.sample_count; }
uint8_t  imu_is_ready(void)            { return g_imu_shadow.ready; }
uint8_t  imu_get_whoami(void)          { return g_imu_shadow.whoami; }
uint8_t  imu_get_init_err(void)        { return g_imu_shadow.init_err; }
uint32_t imu_get_bus_err(void)         { return g_imu_shadow.bus_err; }
uint8_t  imu_get_cfg_ctrl3(void)       { return g_imu_shadow.cfg_ctrl3; }
uint8_t  imu_get_cfg_ctrl1_xl(void)    { return g_imu_shadow.cfg_ctrl1_xl; }
uint8_t  imu_get_cfg_ctrl2_g(void)     { return g_imu_shadow.cfg_ctrl2_g; }
uint8_t  imu_get_cfg_post_ctrl1(void)  { return g_imu_shadow.cfg_post_ctrl1; }
uint8_t  imu_get_cfg_post_ctrl2(void)  { return g_imu_shadow.cfg_post_ctrl2; }
uint8_t  imu_get_cfg_post_ctrl8(void)  { return g_imu_shadow.cfg_post_ctrl8; }
uint8_t  imu_get_cfg_post_ctrl6(void)  { return g_imu_shadow.cfg_post_ctrl6; }
uint8_t  imu_get_cfg_fca(void)         { return g_imu_shadow.cfg_fca; }
uint8_t  imu_get_sflp_en_a(void)       { return g_imu_shadow.sflp_en_a; }
uint8_t  imu_get_sflp_init_a(void)     { return g_imu_shadow.sflp_init_a; }
uint8_t  imu_get_sflp_exec_status(void){ return g_imu_shadow.sflp_exec_status; }
uint8_t  imu_get_sflp_fifo_en_a(void)  { return g_imu_shadow.sflp_fifo_en_a; }
uint8_t  imu_get_fifo_status1(void)    { return g_imu_shadow.fifo_status1; }
uint8_t  imu_get_fifo_status2(void)    { return g_imu_shadow.fifo_status2; }
uint8_t  imu_get_sflp_odr_rdbk(void)   { return g_imu_shadow.sflp_odr_rdbk; }
uint8_t  imu_get_page_sel_rdbk(void)   { return g_imu_shadow.page_sel_rdbk; }
uint8_t  imu_get_cfg_ctrl3_boot(void)  { return g_imu_shadow.cfg_ctrl3_boot; }
uint8_t  imu_get_status_raw(void)      { return g_imu_shadow.status_raw; }
uint8_t  imu_get_xlda(void)            { return g_imu_shadow.status_raw & IMU_XLDA; }
uint8_t  imu_get_gda(void)             { return g_imu_shadow.status_raw & IMU_GDA; }
uint8_t  imu_get_tda(void)             { return g_imu_shadow.status_raw & IMU_TDA; }
...
void imu_set_temp(int16_t temp)               { g_imu_shadow.temp = temp; }
void imu_set_timestamp_ms(uint32_t ts)         { g_imu_shadow.timestamp_ms = ts; }
void imu_inc_sample_count(void)                { g_imu_shadow.sample_count++; }
void imu_set_ready(uint8_t ready)             { g_imu_shadow.ready = ready; }
void imu_set_whoami(uint8_t whoami)           { g_imu_shadow.whoami = whoami; }
void imu_set_init_err(uint8_t err)            { g_imu_shadow.init_err = err; }
void imu_inc_bus_err(void)                    { g_imu_shadow.bus_err++; }
void imu_set_cfg_readback(uint8_t c3, uint8_t c1, uint8_t c2)
{
    g_imu_shadow.cfg_ctrl3   = c3;
    g_imu_shadow.cfg_ctrl1_xl = c1;
    g_imu_shadow.cfg_ctrl2_g  = c2;
}
void imu_set_cfg_post_sflp(uint8_t c1, uint8_t c2, uint8_t c8, uint8_t c6)
{
    g_imu_shadow.cfg_post_ctrl1 = c1;
    g_imu_shadow.cfg_post_ctrl2 = c2;
    g_imu_shadow.cfg_post_ctrl8 = c8;
    g_imu_shadow.cfg_post_ctrl6 = c6;
}
void imu_set_cfg_ctrl3_boot(uint8_t val)   { g_imu_shadow.cfg_ctrl3_boot = val; }
void imu_set_cfg_fca(uint8_t val)           { g_imu_shadow.cfg_fca = val; }
...
{
    g_imu_shadow.sflp_en_a        = en_a;
    g_imu_shadow.sflp_init_a      = init_a;
    g_imu_shadow.sflp_exec_status  = exec;
    g_imu_shadow.sflp_fifo_en_a   = fifo_ena;
    g_imu_shadow.fifo_status1     = fs1;
    g_imu_shadow.fifo_status2     = fs2;
    g_imu_shadow.sflp_odr_rdbk   = odr_rdbk;
    g_imu_shadow.page_sel_rdbk   = pg_sel;
}
void imu_set_status(uint8_t status_raw)       { g_imu_shadow.status_raw = status_raw; }
